<%@ page language="java" contentType="text/html; charset=ISO-8859-1" pageEncoding="ISO-8859-1"%>

<%@ page isELIgnored="false"%>
<%@taglib uri="http://java.sun.com/jsp/jstl/core"  prefix="c"%>
<%@ page errorPage="error.jsp" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<meta name='viewport' content='width=device-width, initial-scale=1'>
<title>Student list</title>
<style>
table{
	width : 100%;
	border-collapse: collapse; 
	border : 1px solid;
    border: 1px solid black;
    background-color : white;
    opacity: 0.8;
    margin-top : 20px;
}
th, td {
  padding: 15px;
  text-align: left;
  border-bottom: 1px solid black;
}
td:nth-child(1){
  background-color: voilet;
}
tr:nth-child(even) {
	background-color: rgb(213, 219, 118);
}
tr:hover {
	background-color: lightgray;
}
a{
	text-decoration:none
}
button{
	width : 80px;
	font-size : 13px;
	font-weight : bold;
	border : none;
	padding : 5px 10px;
	border-radius : 5px;
	display : flex;
	justify-content : center;
	align-items : center;
}
svg{
	font-weight : bold;
	width : 12px; 
	margin-right : 5px;
}
#edit{
	padding : 5px;
	background-color: lightgreen;
}
#delete{
	background-color: red;
}
#options{
	width : 100%;
	display : flex;
	justify-content : space-between;
}
#options input{
	outline : none;
	border-radius : 10px;
}
</style>
</head>
<body>
	<div style="width:80%;margin:auto;">
			<h1>Student Data</h1>
			
			<div id="options">
				<div id="add">
					<a href="addStudent">
						<button><svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512"><path d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z"/></svg>
							Add
						</button>
					</a>
				</div>
			</div>
		
			<table border=1>
			<tr>
				<td><b>Roll No</b></td>
				<td><b>First Name</b></td>
				<td><b>Last Name</b></td>
				<td><b>Gender</b></td>
				<td><b>Email Id</b></td>
				<td><b>Percentage</b></td>
				<td><b>Edit</b></td>
				<td><b>Delete</b></td>
			</tr>
			<c:forEach var="student" items="${studentList}">
				<tr>
					<td>${student.STU_ROLLNO}</td>
					<td>${student.STU_FNAME}</td>
					<td>${student.STU_LNAME}</td>
					<td><%-- ${student.STU_GENDER} --%><%="null" %></td>
					<td>${student.STU_EMAIL}</td>
					<td>${student.STU_PERCENTAGE}</td>
					<%-- 
					<td><a href="editStudent?student_rollno=${student.STU_ROLLNO}" id=""><button id="edit">edit</button></a></td> 
					--%>
					<td>
						<a href="editStudent?student_rollno=${student.STU_ROLLNO}" id="stno">
							<button id="edit">
								<svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512"><path d="M471.6 21.7c-21.9-21.9-57.3-21.9-79.2 0L362.3 51.7l97.9 97.9 30.1-30.1c21.9-21.9 21.9-57.3 0-79.2L471.6 21.7zm-299.2 220c-6.1 6.1-10.8 13.6-13.5 21.9l-29.6 88.8c-2.9 8.6-.6 18.1 5.8 24.6s15.9 8.7 24.6 5.8l88.8-29.6c8.2-2.7 15.7-7.4 21.9-13.5L437.7 172.3 339.7 74.3 172.4 241.7zM96 64C43 64 0 107 0 160V416c0 53 43 96 96 96H352c53 0 96-43 96-96V320c0-17.7-14.3-32-32-32s-32 14.3-32 32v96c0 17.7-14.3 32-32 32H96c-17.7 0-32-14.3-32-32V160c0-17.7 14.3-32 32-32h96c17.7 0 32-14.3 32-32s-14.3-32-32-32H96z"/></svg>
								Edit 
							</button>
						</a>
					</td>
					<td>
						<a href="deleteStudent?student_rollno=${student.STU_ROLLNO}" onclick="return confirm('Do you want to delete the record.')">
							<button id="delete">
								<svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512"><path d="M135.2 17.7L128 32H32C14.3 32 0 46.3 0 64S14.3 96 32 96H416c17.7 0 32-14.3 32-32s-14.3-32-32-32H320l-7.2-14.3C307.4 6.8 296.3 0 284.2 0H163.8c-12.1 0-23.2 6.8-28.6 17.7zM416 128H32L53.2 467c1.6 25.3 22.6 45 47.9 45H346.9c25.3 0 46.3-19.7 47.9-45L416 128z"/></svg>
								Delete
							</button>
						</a>
					</td>
				</tr>
			</c:forEach>
		</table>
		
		<i class='fas fa-pen-square'></i>
	</div>
	<script type="text/javascript">
		var n =  document.getElementById("stno").innerText;
		function deleteFun(n){
			console.log(n);
			var abb = document.getElementById("delete");
			if (confirm("Are sure want to delete!") == true) {
	      		 x = "OK!";
			} else {
				x = "Cancel!";
			}
			if(x == "OK!"){
				abb.href = "deleteStudent"+"?"+"student_rollno="+n;
				console.log("href : "+abb.href);
			}  
		}
	</script>
</body>
</html>